...
const Note = () => {
  useEffect(() => {
    const userToken = Cookies.get('UserToken');Axios.get(`/note/${noteId}`, { headers: { Authorization: `Bearer ${userToken}` } }).then((res) => { setNote(res.data.data);});}, []);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        setStatus({ msg: 'Delete note successfully', severity: 'success' });
        navigate(-1);
      }
    } catch (error) {
      if (error instanceof AxiosError && error.response) {
        ...
        setStatus({ msg: error.message, severity: 'error' });
      }
    }
  };
